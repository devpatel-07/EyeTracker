# Before cv2: OpenCV may load Qt; matplotlib must match.
import matplotlib
...
# ---------------------------------------------------------------------------
# Servo infrastructure — drives to the gaze INTERSECTION POINT (world target)
# ---------------------------------------------------------------------------
#
# The servo consumes `target_head` from gaze_world.compute_world_gaze_projected():
# a 3D point (x, y, z) in head frame showing where the subject is looking.
#
# Yaw to that point:  atan2(target_x, target_z).
#
# Calibration records this yaw at LEFT / CENTER / RIGHT gaze and linearly maps
# it to pulse widths. Tracking uses the same yaw computation for consistency.
#
# Move-and-settle: after sending a pulse, we block further commands for
# SERVO_SETTLE_S seconds so the arm can physically reach position before
# we sample the next gaze target.
#
# Env vars:
#   EYE_TRACKER_SERVO_PORT         serial port (required to enable servo)
#   EYE_TRACKER_SKIP_SERVO_CALIB   nonempty to skip calibration
#   EYE_TRACKER_SERVO_SETTLE_S     seconds to wait after each send (default 0.3)
#   EYE_TRACKER_SERVO_MIN_STEP     minimum pulse µs delta to bother sending (default 5)
#   EYE_TRACKER_SERVO_DEBUG_EVERY  print debug every N sends (default 0 = silent)
# ---------------------------------------------------------------------------
...
def yaw_from_target(target_head):
    """Compute yaw (rad) from a 3D gaze target point in head frame: atan2(x, z)."""
    if target_head is None:
        return None
    t = np.asarray(target_head, dtype=float).ravel()
    if t.size < 3:
        return None
    tx, tz = float(t[0]), float(t[2])
    if abs(tx) + abs(tz) < 1e-9:
        return None
    return math.atan2(tx, tz)


class CalibratedServoYawMap:
    """Maps gaze-target yaw (rad) to servo pulse using three calibration samples."""

    def __init__(self, y_left, y_center, y_right,
    ...
    def pulse_from_yaw(self, yaw_rad):
        ...
        l = self.y_l - self.y_c
        r = self.y_r - self.y_c
        if abs(l) < 1e-6 or abs(r) < 1e-6 or l * r >= 0:
            return None
        s = y - self.y_c
        if s * l >= 0:
            t = s / l
...
def gaze_yaw_to_servo_pulse_us(yaw_rad):
    """Fallback linear map when calibration was skipped. Uses env vars."""
...
class TargetServoSerialBridge:
    """Send pulse commands to ESP firmware given a 3D gaze target.

    Move-and-settle model:
      - Given target_head (x, y, z), compute yaw and corresponding pulse.
      - Send "<pulse>\\n" over serial, then block further sends for
        SERVO_SETTLE_S so the servo can physically arrive before the next
        sample is even considered.
      - update_from_target() calls during the settle window are counted
    ...
    def __init__(self, port, yaw_calibration=None):
        ...
        self._settle_s = max(0.0, _env_float("EYE_TRACKER_SERVO_SETTLE_S", 0.3))
        self._min_delta = max(0, int(_env_float("EYE_TRACKER_SERVO_MIN_STEP", 5)))
    ...
    def shutdown_report_lines(self):
        st = self._st
        lines = [
            "Servo bridge (TargetServoSerialBridge):",
            f"  update() calls:        {st['updates']}",
            f"  target missing:        {st['target_none']}",
            f"  yaw missing:           {st['yaw_none']}",
            f"  pulse map invalid:     {st['pulse_invalid']}",
            f"  skipped (settling):    {st['skip_settle']}",
            f"  skipped (min step):    {st['skip_min_step']}",
            f"  serial writes OK:      {st['writes']}",
            f"  serial write errors:   {st['write_errors']}",
            f"  settle_s={self._settle_s:g}  min_step={self._min_delta}",
        ]
        if st["last_written_pulse"] is not None:
            lines.append(f"  last value written:    {st['last_written_pulse']} us")
        cal = self._yaw_cal
        if cal is not None:
            lines.append(f"  calibration: y_l={cal.y_l:.4f} y_c={cal.y_c:.4f} y_r={cal.y_r:.4f} rad")
            lines.append(f"               -> {cal.p_l}/{cal.p_c}/{cal.p_r} us")
        else:
            lines.append("  calibration: (none — using fallback map)")
        return lines

    def update_from_target(self, target_head):
        """Given a 3D gaze target in head frame, possibly move the servo to it."""
        st = self._st
        st["updates"] += 1

        # Respect settle window even when target is missing.
        now = time.monotonic()
        if self._last_sent is not None and (now - self._last_send_mono) < self._settle_s:
            st["skip_settle"] += 1
            return
        ...
        yaw = yaw_from_target(target_head)
        if yaw is None:
            st["yaw_none"] += 1
            return
        ...
        if self._last_sent is not None and abs(pulse - self._last_sent) < self._min_delta:
            st["skip_min_step"] += 1
            return

        try:
            self._ser.write(f"{pulse}\n".encode("ascii"))
        except _pyserial.SerialException:
            st["write_errors"] += 1
            return

        st["writes"] += 1
        st["last_written_pulse"] = pulse
        self._last_sent = pulse
        self._last_send_mono = now

        if self._debug_every > 0:
            self._debug_i += 1
            if self._debug_i % self._debug_every == 0:
                print(f"[servo] target=({target_head[0]:+.2f}, {target_head[1]:+.2f}, {target_head[2]:+.2f})  "
                      f"yaw={yaw:+.3f}  pulse={pulse}")


# ---------------------------------------------------------------------------
# Servo calibration: uses the SAME world-frame yaw we'll use at runtime.
# ---------------------------------------------------------------------------

def _calibration_tick(ret_l, frame_l, ret_r, frame_r, mirror_mode, state_l, state_r):
    """Run one synchronous dual-eye detection step for calibration.
    Returns per-eye directions and unrotated display frames."""
    ...
    if ret_l:
        fd = crop_to_aspect_ratio(frame_l.copy())
        dp = get_darkest_area(fd)
        if dp is not None:
            gray = cv2.cvtColor(fd, cv2.COLOR_BGR2GRAY)
            dpv = gray[dp[1], dp[0]]
            th_s = mask_outside_square(apply_binary_threshold(gray, dpv, 5),  dp, 250)
            th_m = mask_outside_square(apply_binary_threshold(gray, dpv, 15), dp, 250)
            th_r_ = mask_outside_square(apply_binary_threshold(gray, dpv, 25), dp, 250)
            _, l_direction = process_frames(th_s, th_m, th_r_, fd, gray, dp, False, False, state=state_l)
            disp_l = fd

    if ret_r:
        fd = crop_to_aspect_ratio(frame_r.copy())
        dp = get_darkest_area(fd)
        if dp is not None:
            gray = cv2.cvtColor(fd, cv2.COLOR_BGR2GRAY)
            dpv = gray[dp[1], dp[0]]
            ...
            th_m = mask_outside_square(apply_binary_threshold(gray, dpv, 15), dp, 250)
            th_r_ = mask_outside_square(apply_binary_threshold(gray, dpv, 25), dp, 250)
            _, r_direction = process_frames(th_s, th_m, th_r_, fd, gray, dp, False, False, state=state_r)
            disp_r = fd

    return l_direction, r_direction, disp_l, disp_r


def run_interactive_servo_yaw_calibration(cap_l, cap_r, mirror_mode):
    """Capture gaze-target yaw at LEFT / CENTER / RIGHT.

    Returns:
        CalibratedServoYawMap  on success
        False                  if user pressed S to skip (use fallback map)
        None                   if user pressed Q or streams ended
    """
    ...
    instructions = [
        ("CENTER", "Look STRAIGHT AHEAD. Hold steady, then press SPACE."),
        ("LEFT",   "Look as far LEFT as comfortable. Hold steady, then press SPACE."),
    ...
    for tag, msg in instructions:
        ...
        while True:
            ...
            l_dir, r_dir, disp_l, disp_r = _calibration_tick(
                ret_l, frame_l, ret_r, frame_r, mirror_mode, cal_l, cal_r)

            y = None
            if l_dir is not None and r_dir is not None:
                result = gaze_world.compute_world_gaze_projected(l_dir, r_dir)
                if result is not None:
                    y = yaw_from_target(result['target_head'])
            buf.append(y)
            if len(buf) > buf_max:
                buf.pop(0)
            ...
            overlay_lines = [
                msg,
                "SPACE=capture  Q=abort  S=skip calibration",
                f"current yaw: {y:+.4f} rad" if y is not None else "current yaw: (waiting for both eyes)",
            ]
            for disp in (disp_l, disp_r):
                if disp is None:
                    continue
                for i, line in enumerate(overlay_lines):
                    cv2.putText(disp, line, (8, 24 + i * 22),
            ...
            if key == ord("q"):
                ...
                return None
            if key == ord("s"):
                print("Calibration skipped — will use fallback mapping.")
                return False
            if key == ord(" "):
                m = _buf_mean()
                if m is None:
                    print("Not enough valid samples yet — keep holding gaze and try SPACE again.")
                    continue
                results[tag] = m
                print(f"  Recorded {tag}: yaw = {m:.4f} rad")
                break

    y_c, y_l, y_r = results["CENTER"], results["LEFT"], results["RIGHT"]
    dl = y_l - y_c
    dr = y_r - y_c
    if dl * dr >= 0:
        print("Calibration failed: LEFT and RIGHT yaw must lie on opposite sides of CENTER.")
        print(f"  center={y_c:.4f}  left={y_l:.4f}  right={y_r:.4f}")
        return False

    cal = CalibratedServoYawMap(y_l, y_c, y_r)
    print(f"Calibration OK: left={y_l:.4f}  center={y_c:.4f}  right={y_r:.4f} rad")
    print(f"                -> pulses {cal.p_l}/{cal.p_c}/{cal.p_r} us")
    return cal
...
def print_tracking_session_shutdown_report(*, reason, elapsed_s, frame_idx,
                                            mirror_mode, servo_bridge,
                                            l_dir_none=0, r_dir_none=0, both_dir_none=0):
    print("\n" + "=" * 64)
...
def run_dual_tracking(src_left, src_right=None, mirror_mode=False):
    ...
    if _servo_port:
        if SERIAL_AVAILABLE:
            try:
                servo_bridge = TargetServoSerialBridge(_servo_port)
                print(f"Base servo serial: {_servo_port}")
    ...
    # --- Calibration (only if servo connected and not skipped via env) ---
    if servo_bridge is not None and not os.environ.get("EYE_TRACKER_SKIP_SERVO_CALIB", "").strip():
        print("\n--- Servo yaw calibration (world-frame gaze target) ---")
        print("  SPACE = capture | S = skip to fallback map | Q = quit app")
        cal = run_interactive_servo_yaw_calibration(cap_l, cap_r, mirror_mode)
        if cal is None:
            servo_bridge.close()
            cap_l.release()
            cap_r.release()
        ...
        # Clear smoothing so tracking starts from a known state.
        gaze_world.reset_smoothing()
    ...
    try:
        while True:
            ...
            # Gaze-vector-miss statistics (for Ctrl+C report)
            if l_direction is None: gaze_stats["l_none"] += 1
            if r_direction is None: gaze_stats["r_none"] += 1
            if l_direction is None and r_direction is None: gaze_stats["both_none"] += 1

            # --- World-space gaze: drives BOTH the visualization AND the servo ---
            result = None
            ...
            # --- Servo: move to gaze target, then settle before sampling again ---
            if servo_bridge is not None:
                try:
                    target_head = result['target_head'] if result is not None else None
                    servo_bridge.update_from_target(target_head)
                except Exception as ex:
                    print(f"Servo update error: {ex}")
